...
export class DependenciaComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number): void {
    ...
    this.dependenciaService
      ...
      .subscribe(
        (res: HttpResponse<IDependencia[]>) => this.onSuccess(res.body, res.headers, pageToLoad),
        () => this.onError()
      );
  }
  ...
  registerChangeInDependencias(): void {
    this.eventSubscriber = this.eventManager.subscribe('dependenciaListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: IDependencia[] | null, headers: HttpHeaders, page: number): void {
